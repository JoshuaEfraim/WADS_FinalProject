...
// const API_URL = "http://localhost:5000/api/admin/ticketDetails"
...
export default function TicketDetails() {
  ...
  useEffect(() => {
    const fetchTicket = async () => {
      try {
        setLoading(true)
        const [ticketDetailsRes, ticketRepliesRes] = await Promise.all([
          axios.get(`${API_URL}/${id}`),
          axios.get(`http://localhost:5000/api/tickets/ticketReply/${id}`)
        ]);
        
        const ticketData = {
          ...ticketDetailsRes.data,
          replies: ticketRepliesRes.data.ticket.replies
        };
        
        setTicket(ticketData)
        setForm({ status: ticketData.status, priority: ticketData.priority })
        setLoading(false)
      } catch (err) {
        setError("Failed to fetch ticket details")
        setLoading(false)
