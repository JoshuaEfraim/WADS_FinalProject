...
export default function TicketHistory() {
  ...
  async function loadHistory(page) {
    setLoading(true)
    try {
      // Fetch “all resolved tickets” for the admin (page is optional)
      const res = await fetch(
        `http://localhost:5000/api/tickets/history/admin?page=${page}`
      )
      const data = await res.json()
      if (Array.isArray(data.tickets)) {
        setTickets(data.tickets)
        // Assume backend returns entire list; we do simple 10-per-page slicing on frontend
  ...
  // Apply search + sort on the full array
                  ...
                  >
                    ...
                    <TableCell className="text-center">
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          navigate(`/admin/ticket-reply/${t._id}`)
                        }
                      >
                        View
                      </Button>
